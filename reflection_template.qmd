...
```{r}
#| label: dvs-3-2
#This code is from lab 3 challenge and I have updated it such that I included annotations in the graph to better visualize which section of the stacked bar graph is which. This was not included in the revisions but added here.
...
```{r}
#| label: dvs-4-across
#decieded to wait for a future lab
```
...
```{r}
#| label: dvs-6-2
#This code originates from Lab 3 Challenge. Here, I clean up the table such that it is easier to understand and read. In particular, I made the question number the intended number as opposed to having an additional 900 added on. 
evals |>
  mutate(sex = gender,
         teacher_id = as.factor(teacher_id),
         question_no = question_no-900) |>
...
```{r}
#| label: dsm-2-1
#This code is from Lab 3 challenge. In here, I run a chi squared test between the seniority level of professors and the SET level average for question 3 (a review of activities in their class). 
